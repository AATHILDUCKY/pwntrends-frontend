...
export default function FollowingPage() {
  ...
  const [committedSearch, setCommittedSearch] = useState(""); // actual query
  ...
  const loadFollowing = useCallback(
    async (opts: { replace: boolean; q?: string }) => {
      ...
      const skip = opts.replace ? 0 : following.length;
      ...
      try {
        ...
        if (opts.replace) setFollowing(data);
        else setFollowing((prev) => [...prev, ...data]);

        setHasMore(data.length === PAGE_SIZE);
      } catch (err: any) {
        console.error(err);
        setError(
          err?.response?.data?.detail || "Failed to load following list."
        );
        if (opts.replace) setFollowing([]);
        setHasMore(false);
      } finally {
        setLoading(false);
  ...
  // load on username or committed search change
  useEffect(() => {
    if (!username) return;
    loadFollowing({ replace: true, q: committedSearch });
  }, [username, committedSearch, loadFollowing]);
  ...
  const headerSubtitle = useMemo(() => {
    if (committedSearch) return `Results for “${committedSearch}”`;
    return "People this profile follows";
  }, [committedSearch]);

  if (!username) {
    return <p className="text-xs text-slate-400">No username provided.</p>;
  }
      ...
      <header className="rounded-2xl border border-slate-800 bg-gradient-to-b from-slate-950 to-slate-900/50 p-4 sm:p-5">
        ...
        <div className="mt-3 sm:mt-4">
          <h1 className="text-base sm:text-lg font-semibold text-white">
            <span className="text-primary-300">@{username}</span> is following
          </h1>
        ...
        {hasMore && following.length > 0 && (
          <div className="pt-1 sm:pt-2 flex justify-center">
            <button
              type="button"
              onClick={() =>
                loadFollowing({ replace: false, q: committedSearch })
              }
